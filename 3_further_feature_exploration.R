#### getting organized again if needed ####
...
all <- rbind.data.frame(top100_2016f, top100_2017f, top100_2018f, top100_2019f, top100_2020f)
...
ggplot(all, aes(danceability)) +
  geom_histogram() +
  facet_wrap(~listYear) +
  theme_minimal()

ggplot(all, aes(energy)) +
  geom_histogram() +
  facet_wrap(~listYear) +
  theme_minimal()

ggplot(all, aes(loudness)) +
  geom_histogram() +
  facet_wrap(~listYear) +
  theme_minimal()

ggplot(all, aes(speechiness)) +
  geom_histogram() +
  facet_wrap(~listYear) +
  theme_minimal()

ggplot(all, aes(acousticness)) +
  geom_histogram() +
  facet_wrap(~listYear) +
  theme_minimal()

ggplot(all, aes(instrumentalness)) +
  geom_histogram() +
  facet_wrap(~listYear) +
  theme_minimal()

ggplot(all, aes(liveness)) +
  geom_histogram() +
  facet_wrap(~listYear) +
  theme_minimal()

ggplot(all, aes(valence)) +
  geom_histogram() +
  facet_wrap(~listYear) +
  theme_minimal()

ggplot(all, aes(tempo)) +
  geom_histogram() +
  facet_wrap(~listYear) +
  theme_minimal()
...
source("helper_code/fix_ggradar.R") ## I made some modifications for my own purposes
# https://github.com/sastoudt/groove_is_in_the_heart_and_data/blob/master/fix_ggradar.R

source("helper_code/helper_plot.R") ## using source code from ggradar, adapted for my own purposes
...
#### tables - go look at songs contributing to these pieces ####
